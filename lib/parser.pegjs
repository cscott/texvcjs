...
start
  = _ t:tex_expr
    { assert.ok(t instanceof ast.Tex.ARRAY); return lst2arr(t); }
...
tex_expr
  ...
  / e1:ne_expr name:FUN_INFIX e2:ne_expr EOF
    { return ast.Tex.ARRAY(ast.Tex.INFIX(name, lst2arr(e1), lst2arr(e2))); }
  / e1:ne_expr f:FUN_INFIXh e2:ne_expr EOF
    { return ast.Tex.ARRAY(ast.Tex.INFIXh(f[0], f[1], lst2arr(e1), lst2arr(e2)));}

expr
  = ne_expr
  / ""
    { return ast.Tex.ARRAY(); }

ne_expr
  = h:lit_aq t:expr
    { return ast.Tex.ARRAY(h, t); }
  / h:litsq_aq t:expr
    { return ast.Tex.ARRAY(h, t); }
  / d:DECLh e:expr
    { return ast.Tex.ARRAY(ast.Tex.DECLh(d[0], lst2arr(e))); }
litsq_aq
  = litsq_fq
  / litsq_dq
  / litsq_uq
...
lit_fq
  ...
  / l1:lit_dqn SUP l2:lit
    { return ast.Tex.FQ(ast.Tex.ARRAY(),l1[1], l2); }
...
lit_uqn
  = SUP l:lit
    { return ast.Tex.UQ(ast.Tex.ARRAY(),l); }
lit_dqn
  = SUB l:lit
    { return ast.Tex.DQ(ast.Tex.ARRAY(),l); }
...
lit
  ...
  / b:BIG SQ_CLOSE              { return ast.Tex.BIG(b,  "]"); }
  / l:left e:expr r:right       { return ast.Tex.LR(l, r, lst2arr(e)); }
  / name:FUN_AR1opt e:expr_nosqc SQ_CLOSE l:lit /* must be before FUN_AR1 */
    { return ast.Tex.FUN2sq(name, ast.Tex.CURLY(lst2arr(e)), l); }
  ...
  / CURLY_OPEN e:expr CURLY_CLOSE
    { return ast.Tex.CURLY(lst2arr(e)); }
  / CURLY_OPEN e1:ne_expr name:FUN_INFIX e2:ne_expr CURLY_CLOSE
    { return ast.Tex.INFIX(name, lst2arr(e1), lst2arr(e2)); }
  / CURLY_OPEN e1:ne_expr f:FUN_INFIXh e2:ne_expr CURLY_CLOSE
    { return ast.Tex.INFIXh(f[0], f[1], lst2arr(e1), lst2arr(e2)); }
...
alignat_spec
  = CURLY_OPEN num:([0-9]+ { return text(); }) _ CURLY_CLOSE
    { return ast.Tex.CURLY(ast.Tex.ARRAY(ast.Tex.LITERAL(num))); }
...
chem_lit
  = CURLY_OPEN e:chem_sentence CURLY_CLOSE               { return ast.Tex.CURLY(lst2arr(e)); }

chem_sentence =
    _ p:chem_phrase " " s:chem_sentence                  { return ast.Tex.ARRAY(p,ast.Tex.ARRAY(ast.Tex.LITERAL(" "),s)); } /
    _ p:chem_phrase _                                    { return ast.Tex.ARRAY(p,ast.Tex.ARRAY()); }

chem_phrase =
    m:"(^)"                                              { return ast.Tex.LITERAL(m); } /
    m:chem_word n:CHEM_SINGLE_MACRO                      { return ast.Tex.CHEM_WORD(m, ast.Tex.LITERAL(n)); }/
    m:chem_word                                          { return m; } /
    m:CHEM_SINGLE_MACRO                                  { return ast.Tex.LITERAL(m); } /
    m:"^"                                                { return ast.Tex.LITERAL(m); }

chem_word =
    m:chem_char n:chem_word_nt                           { return ast.Tex.CHEM_WORD(m, n); } /
    m:CHEM_SINGLE_MACRO n:chem_char_nl o:chem_word_nt    { return ast.Tex.CHEM_WORD(ast.Tex.CHEM_WORD(ast.Tex.LITERAL(m), n), o); }
...
chem_char =
    ...
    c:CHEM_LETTER                                        { return ast.Tex.LITERAL(c) }

chem_char_nl =
    m:chem_script                                        { return m;} /
    CURLY_OPEN c:chem_text CURLY_CLOSE                   { return ast.Tex.CURLY(ast.Tex.ARRAY(c)); } /
    BEGIN_MATH c:expr END_MATH                           { return ast.Tex.DOLLAR(lst2arr(c)); }/
    name:CHEM_BOND l:chem_bond                           { return ast.Tex.FUN1(name, l); } /
    m:chem_macro                                         { return m; } /
    c:CHEM_NONLETTER                                     { return ast.Tex.LITERAL(c) }

chem_bond
 = CURLY_OPEN e:CHEM_BOND_TYPE CURLY_CLOSE               { return ast.Tex.CURLY(ast.Tex.ARRAY(ast.Tex.LITERAL(e))); }

chem_script =
    a:CHEM_SUPERSUB b:CHEM_SCRIPT_FOLLOW                 { return ast.Tex.CHEM_WORD(ast.Tex.LITERAL(a), ast.Tex.LITERAL(b)); } /
    a:CHEM_SUPERSUB b:chem_lit                           { return ast.Tex.CHEM_WORD(ast.Tex.LITERAL(a), b); } /
    a:CHEM_SUPERSUB BEGIN_MATH b:expr END_MATH           { return ast.Tex.CHEM_WORD(ast.Tex.LITERAL(a), ast.Tex.DOLLAR(lst2arr(b))); }
...
chem_text = cs:boxchars+                                 { return ast.Tex.LITERAL(cs.join('')); }
CHEM_COLOR = "{" _ name:alpha+ _ "}" _                   { return ast.Tex.LITERAL(name.join('')); }
