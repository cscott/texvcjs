...
LITERAL
 ...
 / f:generic_func &{ return options.mathjax && tu.other_literals2[f]; } _ // from Texutil.find(...)
   { return ast.RenderT.TEX_ONLY("\\mathrm {" + f + "} "); }
 / mathrm:generic_func &{ return options.mathjax && mathrm === "\\mathrm"; } _
   "{" f:generic_func &{ return options.mathjax && tu.other_literals2[f]; } _ "}" _
   /* make sure we can parse what we emit */
   { return options.mathjax && ast.RenderT.TEX_ONLY("\\mathrm {" + f + "} "); }
...
DELIMITER
 ...
 / f:generic_func &{ return tu.other_delimiters2[f]; } _ // from Texutil.find()
   { var p = parse(tu.other_delimiters2[f]);
